    ...
    <div className="max-w-6xl mx-auto pb-20">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm transition-all hover:shadow-md group">
          ...
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Total Civitas</p>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm transition-all hover:shadow-md group">
          ...
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Guru</p>
          <p className="text-2xl font-bold text-slate-900 mt-1">{stats.guru}</p>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm transition-all hover:shadow-md group">
          ...
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Karyawan/Staf</p>
          <p className="text-2xl font-bold text-slate-900 mt-1">{stats.karyawan}</p>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm transition-all hover:shadow-md group">
          ...
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Pegawai Tetap</p>
          <p className="text-2xl font-bold text-slate-900 mt-1">{stats.tetap}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 md:p-6 mb-8">
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 md:pb-0 scrollbar-hide">
            ...
            <button className="flex items-center justify-center gap-2 px-5 py-2.5 border border-slate-200 rounded-xl text-slate-600 font-bold text-sm hover:bg-slate-50 transition-all">
      ...
      <div className="space-y-4">
        {filteredCivitas.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200">
            ...
            <h3 className="text-xl font-bold text-slate-800">Civitas tidak ditemukan</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredCivitas.map((civitas) => (
              <div 
                key={civitas.id}
                className="bg-white rounded-2xl border border-slate-200 overflow-hidden transition-all hover:border-green-300 hover:shadow-xl group"
              >
                <div className="p-6">
                  <div className="flex items-start justify-between mb-5">
                    <div className="relative">
                      <div className={`h-16 w-16 rounded-2xl flex items-center justify-center text-2xl font-bold transition-all group-hover:scale-105 ${
                        civitas.gender === "Laki-laki" ? "bg-blue-50 text-blue-600" : "bg-rose-50 text-rose-600"
                      }`}>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-xs font-medium text-slate-400">{civitas.role} • {civitas.unit}</span>
                      <span className="h-1 w-1 rounded-full bg-slate-300"></span>
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                        civitas.status === "Tetap" ? "bg-green-100 text-green-700" : 
                        civitas.status === "Kontrak" ? "bg-blue-100 text-blue-700" : 
