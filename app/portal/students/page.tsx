    ...
    <div className="max-w-4xl mx-auto">
      <header className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl font-bold text-slate-800 tracking-tight">Murid Saya</h1>
        </div>
        <p className="text-slate-500">Daftar putra/putri yang berada di bawah perwalian Anda.</p>
      </header>
...
{loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border-2 border-dashed border-slate-200">
          ...
          <h3 className="text-lg font-semibold text-slate-800">Belum ada murid terdaftar</h3>
          <p className="text-slate-500 mt-1 text-sm">Hubungi administrator untuk mendaftarkan murid.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {students.map(student => (
            <div
              key={student.id}
              className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex items-start gap-4 hover:border-green-300 hover:shadow-md transition-all"
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap mb-2">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                    student.status === "Aktif" ? "bg-green-100 text-[#1a7a4a]" : "bg-slate-100 text-slate-500"
                  }`}>
                    {student.status}
                  </span>
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className={`px-2.5 py-1 rounded-lg text-xs font-bold ${UNIT_COLORS[student.unit] ?? "bg-slate-100 text-slate-600"}`}>
                    {student.unit}
                  </span>
                  <span className="text-sm text-slate-600 font-medium">Kelas {student.grade}</span>
                  {student.nisn && (
                    <span className="text-xs text-slate-400">NISN: {student.nisn}</span>
                  )}
                </div>
              </div>
            </div>
